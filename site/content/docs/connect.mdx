...
In a terminal, the command asks two questions. First it shows a transport menu, with local stdio preselected and remote and OAuth also on offer. Then it shows an arrow-key multiselect of supported clients. Detected clients are preselected. Detection checks for the client's configuration file or its marker directory.
...
In stdio mode, the entry runs `lific --db <absolute-db-path> mcp` and carries the minted token in the client's environment field as `LIFIC_TOKEN`. The field name follows the client: `environment` for OpenCode and `env` for Claude Code, Claude Desktop, Codex, and Zed. If a selected client's documented schema has no environment field, `lific connect` skips that client before minting its bot or key. It never silently drops an agent identity into an operator session.
...
### Codex, Claude, and Zed compatibility evidence

The connection tests pin complete, exact entries rather than checking a few fields:

- Codex covers bearer HTTP, header-less OAuth, agent-bound stdio, and tokenless operator stdio. Codex CLI's config parser accepts the generated HTTP and stdio forms. See the [Codex MCP configuration reference](https://developers.openai.com/codex/mcp/).
- Claude Code covers the same four lifecycles, and Claude Code's isolated `mcp add-json` parser accepts the generated remote and stdio entries. Claude Desktop covers bearer HTTP through `mcp-remote` plus agent-bound and operator stdio; its config-file form does not claim OAuth support. See the [Claude Code MCP reference](https://code.claude.com/docs/en/mcp).
- Zed covers the same four lifecycles with exact fixtures matching Zed's documented `context_servers` schema. This evidence is a faithful schema fixture; the test environment did not have the Zed editor available for a live parser run. See the [Zed MCP reference](https://zed.dev/docs/ai/mcp).

Those fixtures also assert the top-level key, entry name, command, arguments, URL, authorization carrier, and environment field, so a schema or lifecycle drift fails as one comparison.
...
- `LIFIC_API_KEY` is the bearer credential for remote HTTP access. Codex reads it, and `lific doctor` falls back to it when `--key` is omitted.
- `LIFIC_TOKEN` is the identity of a local stdio MCP session. `lific connect --stdio` writes it into the client's environment field; nothing reads it over HTTP.
...
- `--stdio` writes a local command form, `lific --db <absolute-db-path> mcp`, and mints the tool a bot and key that it writes into the entry's environment as `LIFIC_TOKEN`. The client launches Lific as a subprocess and there is no network authentication. This suits a machine where the client and the database live together.
- Remote HTTP with a bearer key. This is the transport a non-interactive run uses when neither transport flag is passed.
