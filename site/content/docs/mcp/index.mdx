...
The repository includes `scripts/mcp-conformance.sh` for the frozen official
July 2026 requirements set. Start Lific, then run the authenticated endpoint by
naming an exported variable that contains a disposable API key:

```bash
export LIFIC_CONFORMANCE_TOKEN='<disposable API key>'
MCP_CONFORMANCE_BEARER_TOKEN_ENV=LIFIC_CONFORMANCE_TOKEN \
  scripts/mcp-conformance.sh http://127.0.0.1:3456/mcp
unset LIFIC_CONFORMANCE_TOKEN
```

The token value is read from that variable and injected by a loopback proxy; it
is not placed in the conformance runner's command line. For an isolated path
token endpoint, pass its complete secret URL directly and do not set the bearer
variable:

```bash
scripts/mcp-conformance.sh \
  'http://127.0.0.1:3456/mcp/<path-token>'
```

The pinned requirements set runs 37 scored server scenarios plus Tasks and
other scenarios that the official runner reports without scoring. Some scored
scenarios assume the suite's everything-server fixture: they call fixture tool
names or exercise completion, prompts, resources, elicitation, sampling, and
roots even though Lific does not advertise those optional capabilities. The
baseline records only those exact check IDs. Successful checks in the same
scenario remain enforced, any unlisted failure is fatal, and a stale baseline
also fails. Run both endpoint forms before release because they have separate
...
Lific prefers Client ID Metadata Documents: a client can use an HTTPS URL with a
path as its `client_id`, and Lific validates the fetched JSON metadata and exact
redirect URI before approval. Dynamic client registration at `/oauth/register`
remains the compatibility path. Registered redirect URIs must use HTTPS, or HTTP
on localhost/loopback for native clients. The July `resource` indicator is
