...
The repository includes `scripts/mcp-conformance.sh` for the official July
2026 server suite. Start Lific, then run:

```bash
scripts/mcp-conformance.sh http://127.0.0.1:3456/mcp
```

The script pins the conformance runner and runs the July scenarios applicable
to Lific's tools-only server surface: stateless requests, tool listing,
completion, protocol errors, DNS rebinding, and HTTP header validation. The
four narrowly baselined stateless checks require diagnostic fixture tools that
Lific does not expose; every other failure remains fatal. Optional MCP
capabilities such as prompts, resources, elicitation, sampling, and tasks are
not advertised by Lific and are not included in this server run.
