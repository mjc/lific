...
The script pins the conformance runner and runs every available server
scenario for `2026-07-28`, including the stateless, header-validation, DNS
rebinding, and caching checks. Its output is the conformance evidence; the
suite's optional scenarios may be reported separately by the runner.
...
remains the compatibility path. Registered redirect URIs must be absolute HTTP
or HTTPS URLs. The July `resource` indicator is required in both authorization
and token requests, and issued access tokens are accepted only for the exact MCP
resource they name.
