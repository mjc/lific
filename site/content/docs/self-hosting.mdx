...
create that directory separately and make it writable by the service user.
...
database and attachments stay under the systemd-managed state directory. The
configuration source may be root-only because systemd loads it as a service
credential.
