...
```shell
lific --config ~/.config/lific/lific.toml start
```
...
The built-in service command uses the current Lific binary and the selected configuration file. Its working directory is the directory containing that configuration file. `lific init --db` persists an absolute database path into the config before installing the service. A later `lific service install --db` is rejected; edit `database.path` instead so the persistent service has one path authority.

## NixOS service

The repository flake exports `nixosModules.default`, which provides a hardened
system service through `services.lific`.
...
```nix
{
  inputs = {
    ...
    lific = {
      url = "github:VoidNullable/lific";
      inputs.nixpkgs.follows = "nixpkgs";
    };
  };

  outputs = { nixpkgs, lific, ... }: {
    nixosConfigurations.example = nixpkgs.lib.nixosSystem {
      system = "x86_64-linux";
      modules = [
        lific.nixosModules.default
        {
          services.lific = {
            enable = true;
            host = "127.0.0.1";
...
The module creates the `lific` system user and group, keeps the database and
attachments under `/var/lib/lific`, loads the configuration as a read-only
systemd credential, and starts a hardened systemd unit. It listens on loopback
by default and does not open the firewall or configure a reverse proxy.

Values under `settings` are evaluated into the Nix store, so they must not
contain secrets. For a secret-bearing configuration, set
`services.lific.configFile` to an absolute runtime path such as
`/run/secrets/lific.toml` and do not set `settings`. In particular, the module
rejects `server.mcp_path_token` under `settings` so that the bearer credential
cannot be copied into the store. The source may remain readable only by root;
systemd presents it read-only to the service at startup.

With generated settings, the application default `backup.dir = "backups"`
resolves beneath the module's `/var/lib/lific` state directory, which the
service user owns and the unit manages with `StateDirectory`. No separate
tmpfiles rule is needed. An external `configFile` remains authoritative for
`backup.dir`; if it selects an absolute directory outside the state directory,
create that directory separately, make it writable by the service user, and
list it in `extraReadWritePaths`:

```nix
services.lific.extraReadWritePaths = [ "/srv/lific-backups" ];
```
...
LoadCredential=lific.toml:/etc/lific/lific.toml
ExecStart=/usr/local/bin/lific --config /run/credentials/lific.service/lific.toml --db /var/lib/lific/lific.db start
...
The configured backup directory must be writable by the `lific` user. The
database and attachments stay under the systemd-managed state directory. With
`ProtectSystem=strict`, add an absolute external backup directory to
`ReadWritePaths` in the unit as well. The configuration source may be root-only
because systemd loads it as a service credential.
