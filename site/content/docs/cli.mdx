...
- `--stdio` writes a local MCP configuration that runs `lific --db <PATH> mcp`. When `--key` is omitted, it mints a per-tool bot and key and writes the key into the entry's environment as `LIFIC_TOKEN`; with `--key`, it reuses the provided key.
...
Exports a project to an output directory. In HTTP mode, this downloads one ZIP archive from the server.
