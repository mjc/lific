...
- `--stdio` writes a local MCP configuration that runs `lific --db <PATH> mcp`. It mints a per-tool bot and key and writes the key into the entry's environment as `LIFIC_TOKEN`.
